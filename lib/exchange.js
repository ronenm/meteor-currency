Exchange = function(to,from,callback) {
  ...
  from = _.map(from, Currency.find_definitive);
  to = _.map(to, Currency.find_definitive);
  ...
  if (typeof(callback)=="function") {
    return Exchange.exec_yql(pairs, function(error, result) {
      if (error) {
    ...
    var result = Exchange.exec_yql(pairs);
    return result.data.query.results.rate;
  }
}

Exchange.exec_yql = function(pairs,callback) {
