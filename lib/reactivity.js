...
Exchange._downloadExchange  = function() {
  Exchange.execYQL(_.keys(Exchange.exchangeCache), _updateExchangeCallback);
}

Exchange.downloadExchangeNow = function(force) {
  var exchangeCache_size = _.keys(Exchange.exchangeCache).length;
  if ((force || exchangeCache_size<=0) && Exchange._exchangeIntervalId) {
    Meteor.clearInterval(Exchange._exchangeIntervalId);
    delete Exchange._exchangeIntervalId;
  }
  if (exchangeCache_size>0 && !Exchange._exchangeIntervalId) {
    Exchange._downloadExchange();
    // It is enough to perform a download every 10 minutes
    Exchange._exchangeIntervalId = Meteor.setInterval(Exchange._downloadExchange,
